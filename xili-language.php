...
*/
# updated 090322 - 0.9.8 - Data model now include default and future sub-group and sorting of languages. 
# Add new hooks to define header metas or language attributes in html tag.	
# updated 090319 - 0.9.7.5 - Detect browser default languages list, fixes W3C, record undefined
...
define('XILILANGUAGE_VER','0.9.8'); /* used in admin UI*/
...
class xili_language {
	...
	function xili_language($metabox = false, $post_ajax = false) {
		...
		$this->langs_group_tax_id = $thegroup[0]->term_taxonomy_id;
		//print_r($thegroup[0]);
		/* default values */
	...
	function head_language_attributes($output) {
		...
		if ($this->langstate == true) {	
			...
			$listlang = array();
			//$listlanguages = get_terms(TAXONAME, array('hide_empty' => false));
			$listlanguages = get_terms_with_order($this->langs_group_id,TAXOLANGSGROUP);
			foreach ($listlanguages as $language) {
	...
	function choice_of_browsing_language() {
		...
		if ($this->browseroption != 'browser') return $this->default_lang; /* in settings UI - after filter to hook w/o UI */
		...
		if (is_array($listofprefs)) {
			...
			foreach($listofprefs as $browserlanguage => $priority) {
					...
					foreach($listlanguages as $sitelanguage) {
						if ($sitelanguage->slug == str_replace('-','_',$browserlanguage)) return $sitelanguage->slug;
					}
			}
			foreach($listofprefs as $browserlanguage => $priority) {
					/* match root languages to give similar in site  - second : two first chars langs*/
					foreach($listlanguages as $sitelanguage) {
						if (str_replace('-','_',$browserlanguage) == substr($sitelanguage->slug,0,2)) return $sitelanguage->slug;
					}	
			}	
		} else {
	...
	function on_load_page() {
			...
			add_meta_box('xili-language-sidebox-3', __('Info','xili-language'), array(&$this,'on_sidebox_2_content'), $this->thehook , 'side', 'core');
	...
	function languages_settings() { 
		...
		switch($action) {
			case 'updateoptions';
				...
				update_option('xili_language_settings', $this->xili_settings);
				$message .= " - ".__('Option is updated.','xili-language')." (=> ".$this->browseroption.") ";
				$actiontype = "add";
				break;
			case 'add':
			...
			case 'edit';
			    ...
			    $language = get_term_withorder ($term_id,$this->langs_group_tax_id);
			...
			case 'edited';
				...
				wp_set_object_terms($theids['term_id'], 'the-langs-group', TAXOLANGSGROUP);
				update_term_order ($theids['term_id'],$this->langs_group_tax_id,$_POST['language_order']);
		...
		add_meta_box('xili-language-sidebox-2', __('Settings','xili-language'), array(&$this,'on_sidebox_3_content'), $this->thehook , 'side', 'core');
		
		/* form datas in array for do_meta_boxes() */
		$data = array('message'=>$message,'messagepost'=>$messagepost,'action'=>$action, 'formtitle'=>$formtitle, 'language'=>$language,'submit_text'=>$submit_text,'cancel_text'=>$cancel_text,'browseroption'=>$this->browseroption);
		?>
	...
	function  on_sidebox_3_content($data) { /* where to choose if browser language preferences is tested or not */
		...
		?>
		<label for="xili_language_check_option" class="selectit"><input id="xili_language_check_option" name="xili_language_check_option" type="checkbox" value="browser"  <?php if($browseroption=='browser') echo 'checked="checked"' ?> /> <?php echo _e('Select by default browser language of visitor', 'xili-language'); ?></label>
	...
	function xili_lang_row() { 	
		/*list of languages*/
		//$listlanguages = get_terms(TAXONAME, array('hide_empty' => false,'get'=>'all'));$this->langs_group_id
		//$listlanguages_id = get_objects_in_term($this->langs_group_id, TAXOLANGSGROUP);
		
		$listlanguages = get_terms_with_order($this->langs_group_id,TAXOLANGSGROUP);
		
		//$listlanguages = wp_get_object_terms($listlanguages_id,TAXONAME,array('orderby' => 'term_order'));
 ...
 *
 */
function get_terms_with_order($group_id,$taxonomy,$order = 'ASC') {
	global $wpdb;
	$term_ids = $wpdb->get_results("SELECT tr.object_id, tr.term_order FROM $wpdb->term_relationships AS tr INNER JOIN $wpdb->term_taxonomy AS tt ON tr.term_taxonomy_id = tt.term_taxonomy_id WHERE tt.taxonomy IN ('".$taxonomy."') AND tt.term_id IN (".$group_id.") ORDER BY tr.term_order $order ");
	$listterms = array();
	foreach ($term_ids as $term_id) {
		$theterm = get_term($term_id->object_id, TAXONAME);
 ...
 * @since 0.9.8
 *
 * update term order in relationships (for terms of langs group)
 *
 ...
 *
 * @param term_id and $taxonomy
 * @return object with term_order
 */
function get_term_withorder ($term_id,$taxonomy) {
	global $wpdb;
	$term = get_term($term_id,TAXONAME,OBJECT,'edit');
	$term->term_order = $wpdb->get_var("SELECT term_order FROM $wpdb->term_relationships WHERE object_id =  $term_id AND term_taxonomy_id = $taxonomy ");
	return $term;
}
