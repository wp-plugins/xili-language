...
define('XILILANGUAGE_VER','1.8.9.1'); /* used in admin UI*/
...
class xili_language {
	...
	function xili_language( $locale_method = false, $show = false ) {
		...
		if(empty($this->xili_settings)) {
			...
			if ( ! isset ( $this->xili_settings['version'] ) || $this->xili_settings['version'] != '1.7') { // repair
				$this->initial_settings ();
				update_option('xili_language_settings', $this->xili_settings);
			}
		}
...
//echo '->'.$id.$langmeta.$targetID.'</br>'; 
	...
	/**
	 * Insert popup in quickedit at end
	 *
	 * @since 1.8.9
	 *
...
function xili_nav_lang_list( $items, $args ) {
	...
	if ( $navmenu_check_option == $args->theme_location ) { 
		...
		$end = xili_language_list( '<li>', '</li>', 'typenav', false, true ) ; // don't display hidden languages 1.8.9.1
