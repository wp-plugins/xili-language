...
/*filters used when querytag is used - see below and functions.php where rules depend from theme*/
...
function xiliml_language_head() {
	...
	if(!is_front_page()) {
		...
		if (is_page()) {
    		...
    		//get var to override the selected lang 
			}}
	} else {
		if ( '' != $wp_query->query_vars[QUETAG] ) {
...
function xiliml_the_other_posts($post_ID,$before = " ", $after = ", ") {
	return xiliml_list_links($post_ID, $before,$after);
}	
function xiliml_list_links($post_ID,$before = " ", $after = ", ") {
	if (function_exists('xili_infunc_list_links')) return xili_infunc_the_other_posts($post_ID, $before,$after);
	/* default here*/
	global $curlang;
	$listlanguages = get_terms(TAXONAME, array('hide_empty' => false));
		foreach ($listlanguages as $language) {
