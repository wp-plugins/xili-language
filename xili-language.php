...
define('XILILANGUAGE_VER', '2.8.3.1'); /* used in admin UI*/
...
class xili_language {
	...
	function get_show_page_on_front_array() {
		...
		$do = ( $this->xili_settings['show_page_on_front_array'] != $front_pages_array ) ? true : false ;
		...
		if ( "" != $this->sublang )  { // see above
			...
			if ($do_it) { // insertion of selection
				...
				if ( $query_object->is_home && $this->show_page_on_front ) {  
					...
					xili_xl_error_log ( $query_object->query_vars['page_id'] . ' <-----+++**** 1240 where home ' .  $query_object->query_vars[QUETAG] . '-' . get_option('page_on_front'));
				}
			...
			} else {	 //xili_xl_error_log ( "****** ici ******" . serialize ( $query_object->query_vars['post_type']) );
