...
define('XILILANGUAGE_VER', '2.8.4.1'); /* used in admin UI*/
...
class xili_language {
	...
	public function __construct( $locale_method = false, $show = false, $class_admin = false ) {
		...
		if( false === $this->xili_settings ) { //1.9.1
			...
			update_option('xili_language_settings', $this->xili_settings);		xili_xl_error_log ( '******XL-adm #179 = '. serialize ( $this->xili_settings ) );		
		} else {
		...
		if ( !$class_admin ) { // filtering only one time 2.6
			...
			if ( !is_admin() ) {
				...
				if ('' != $this->xili_settings['in_nav_menu']) {
						...
						if ( version_compare($wp_version, '3.4.9', '>') ) // new behaviour - add dummy
	...
	function init_vars () { 
		...
		$thegroup = get_terms(TAXOLANGSGROUP, array('hide_empty' => false,'slug' => 'the-langs-group', 'cache_domain' => 'core1'));
		...
		if ( array() == $thegroup ) { /* update langs group 0.9.8 and if from start 2.3.1 */ 
			...
			$list_languages = get_terms(TAXONAME, array('hide_empty' => false, 'get' => 'all', 'cache_domain' => 'core1'));
			...
			if ( array() == $list_languages ) { /*create two default lines with the default language (as in config)*/ 
				...
				$list_languages = get_terms(TAXONAME, array('hide_empty' => false, 'get' => 'all', 'cache_domain' => 'core2'));
				
			}	
			foreach( $list_languages as $language ) {
				wp_set_object_terms( $language->term_id, 'the-langs-group', TAXOLANGSGROUP );
			}
			$thegroup = get_terms( TAXOLANGSGROUP, array('hide_empty' => false,'slug' => 'the-langs-group', 'get' => 'all', 'cache_domain' => 'core2') );
		}
	...
	function get_lang_slug_ids( $i = 'null') {
		$this->alias_mode = ( has_filter ( 'alias_rule', 'xili_trans_slug_qv' ) ) ? true : false  ;
 		$this->xili_settings = get_option( 'xili_language_settings'); // refresh values in admin side - 2.8.3
		$lang_slugs = array() ;
		$lang_names = array() ;
		$lang_full_names = array() ;
		$listlanguages = $this->get_listlanguages( true ); //xili_xl_error_log (' #682 ********* ' );
		...
		foreach ( $listlanguages as $lang) {
			...
			if ( $this->alias_mode ) {  
				...
				$langs_slug_shortqv[$key] = $short; //xili_xl_error_log ($key . '--++o'.$i.'oo++--'.  $short );
				if ( '' != $short ) {
					$langs_shortqv_slug[$short] = $key;
				}
			} else {
				if ( isset( $this->xili_settings['lang_features'][$key]['alias'] ) ) {
					unset ( $this->xili_settings['lang_features'][$key]['alias'] );
					$do = true;
				}
			}
		}
		...
		if ( $this->alias_mode ) { //xili_xl_error_log ('updated_array');
			// 2.8.2
	...
	function get_show_page_on_front_array() {
		...
		if ( is_admin () && $do ) update_option( 'xili_language_settings', $this->xili_settings ); // if changing occurs...
				...
				if ( is_page() ) {
					...
					if ( $this->lang_perma ) {
						...
						if ( isset( $_GET[QUETAG] ) ) {
							$curlang = $this->lang_qv_slug_trans ( $_GET[QUETAG]) ;
						} else {
	...
	function wp_nav_title_text ( $value = "", $itemID = 0 ){
		if ('' != $value) { //xili_xl_error_log ( '#1923 ' . $value . ' - item = ' . $itemID ) ;
			return __( $value, $this->thetextdomain );
		} else {
			return $value;
		}
	...
	function translate_page_on_front_ID ( $original_id ) { 
		...
		if ( $this->lang_perma ) {
			...
			if ( !is_admin() && $this->show_page_on_front ) { 
			 	...
			 	$trans_id = $this->linked_post_in( $original_id, $curlang ) ;  //xili_xl_error_log ( $original_id .'<--2334-->' .$curlang . ' = ' . $trans_id );
			...
			} else { //xili_xl_error_log ('out'); xili_xl_error_log ('ni admin ni front #2340');
				return $original_id;
			}
		
		} else { //xili_xl_error_log ('ni lang perma 2344'); 
			//$showpage = get_option('show_on_front');
			if ( !is_admin() && $this->show_page_on_front ) {
		...
		if ( '' != $this->xili_settings['theme_domain'] ) { 
			...
			if ( is_admin() ) {
				...
				if ( $do ) {
					...
					update_option('xili_language_settings', $this->xili_settings); // ? redondant ?
				}
			}
		} else {
	...
	function theme_switched ( $theme ) { 
		$this->xili_settings['langs_folder'] = ""; //error_log (' ici - ******* ');
		...
		if ( ! $menu && $args->theme_location && ( $locations = get_nav_menu_locations() ) && isset( $locations[ $args->theme_location ] ) )
			...
			foreach ( $language_slugs_list as $slug ) {
				...
				if ( $slug != $default) {
					...
					if ( isset ( $navmenu_check_options[ $args->theme_location.'_'.$slug ] ) && $navmenu_check_options[ $args->theme_location.'_'.$slug ]['navenable'] == 'enable' ) {
...
function xili_nav_lang_list( $items, $args ) {  
	...
	$li_separator = ( '' != $xili_language->xili_settings['nav_menu_separator'] ) ? '<li class="menu-item menu-separator" ><a>'.$xili_language->xili_settings['nav_menu_separator'].'</a></li>' : '';  // 2.8.3 
	
	if ( 0 != strpos( $items, '>xili-language un-visible dummy-menu-item<' )) {
		
		//xili_xl_error_log ( preg_replace ( '/<li(.*)href="#dummy-link(.*)<\/li>/i', '',$items) ) ;
...
function xili_nav_page_list( $items, $args ) { 
	...
	$navmenu_check_options = $xili_language->xili_settings['navmenu_check_options'];
	if ( has_filter( 'xili_nav_page_list' ) ) return apply_filters ( 'xili_nav_page_list',  $items, $args, $navmenu_check_option, $navmenu_check_options );
