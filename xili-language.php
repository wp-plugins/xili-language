...
define('XILILANGUAGE_VER', '2.8.4.3'); /* used in admin UI*/
...
class xili_language {
	...
	var $langs_list_options = array( array('','default'), array('typeone','Type #1'), array('typeonenew','Type for single'), array('navmenu', 'Nav Menu' ), array('navmenu-a', 'Nav Menu (w/o current lang.)' ) , array('navmenu-1', 'Nav Menu Singular' ), array('navmenu-1a', 'Nav Menu Singular (w/o current lang.)' ) ); // type of languages list see options in xili_language_list or navmenu
	...
	function only_parent_construct () {	
		...
		if( false === $this->xili_settings ) { //1.9.1
			...
			if ( ! isset ( $this->xili_settings['version'] ) || $this->xili_settings['version'] != '2.5') { // repair or restart from new
				$this->initial_settings ();
				update_option('xili_language_settings', $this->xili_settings);  
			}
		}
	...
	function xili_language_activate() {
		$this->xili_settings = get_option('xili_language_settings'); xili_xl_error_log('xl first activate');
