...
define('XILILANGUAGE_VER','2.2.2'); /* used in admin UI*/
...
class xili_language {
	...
	function xili_language( $locale_method = false, $show = false ) {
		...
		// since 2.2.0
		add_action( 'admin_bar_init', array(&$this, 'xili_admin_bar_init') ); // add button in admin bar
	...
	function  find_post_script () {
			...
			wp_enqueue_script( 'xili-find-post', plugin_dir_url ( __FILE__ ) . 'js/xili-findposts.dev.js' );
	}
	...
	function posts_join_with_lang( $join, $query_object = null ) {
		...
		if ( '' != $query_object->query_vars[QUETAG] ) {
			if ( '*' == $query_object->query_vars[QUETAG] ) {	// undefined lang
				...
				if ( $query_object->query_vars['category_name'] == "" && $query_object->is_tax && isset( $query_object->query_vars[QUETAG]) && $query_object->query_vars['taxonomy'] == TAXONAME && $this->show_page_on_front ) {
					$join = '';
					$insert_join = false ;
				}
				if ( $query_object->is_tax &&  $query_object->query_vars['category_name'] != '' ) $insert_join = true ;
			}
			//error_log ( '--oooo-join-oooo---'.$join );
	...
	function posts_where_lang( $where, $query_object = null ) {
		...
		if ( '*' == $query_object->query_vars[QUETAG] ) { // undefined 
			...
			if ($query_object->query_vars['caller_get_posts']) { 
				...
				if ( $this->lang_perma ) {
					if ( $query_object->is_page && isset( $query_object->query_vars[QUETAG]) ) {
						//if ( $query_object->query_vars['page_id'] != get_option('page_on_front') ) echo '--!!!--';
						...
						//error_log ( 'query ici '.$where);
						$do_it = true; // all but not home
						
					}
					//print_r( $query_object->query_vars );
					...
					if ( $query_object->is_tax &&  $query_object->query_vars['taxonomy'] == TAXONAME && "" == $query_object->query_vars['category_name'] ) {
						$do_it = false;
						//error_log ( '-----perma--where--lang---'. (int)$query_object->is_tax );
					...
					//error_log ( '-----short-----'. (int)$do_it );
				}
			}
			//error_log ( '-----feed-----'. $query_object->query_vars['feed'] );
			//print_r($query_object);
			if ($do_it) {
				...
				if ( $query_object->is_home && $this->show_page_on_front ) { 
					...
					//error_log ( 'home en cours ');
				}
				//error_log ( 'test lang permalink running ***********');
				if ( $this->lang_perma && $this->show_page_on_front) { // 2.1.1
					if ( $query_object->query_vars[QUETAG] != "" && $query_object->query_vars['taxonomy'] == TAXONAME  ) {
						...
						$query_object->is_singular = true ;
						$query_object->query = array();
						$pid = $this->get_option_wo_xili ('page_on_front') ; //echo $pid;
						...
						//error_log (  $lang.'---- page ----> '.$query_object->query_vars['page_id'] ) ;
						//unset ( $query_object->query_vars['lang']);
						...
						//print_r($wp_query->get_queried_object());
						//error_log (  $lang.'---- page ----> '.get_option ('page_on_front').' >--- pid = '.$query_object->query_vars['page_id'] );
			...
			if ( $this->lang_perma && $this->show_page_on_front ) {
				$first_query = get_option('xili_language_settings_test') ;
				if ( in_array ( $first_query , array_keys ( $this->langs_ids_array ) ) ) { 
					
					$where = preg_replace ( '/(ID) = (\d+) /', '$1 = '.$this->show_page_on_front_array[$first_query]." " , $where ) ; 
					$query_object->query_vars[QUETAG] = $first_query ; 
					//error_log ( '---- preg ---- = '. $first_query );
					delete_option('xili_language_settings_test') ;
				}
				
				//error_log ( get_option ('page_on_front').'>---- no lang ===== where = ID = ' . $wp_query->query_vars['page_id'].'***'.$first_query );	 
	...
	function translate_page_on_front_ID ( $original_id ) {
		...
		if ( $this->lang_perma ) {
			...
			//if ( isset($_GET[QUETAG]) ) {  // $this_curlang is not yet set
			//	$curlang = $_GET[QUETAG] ; //$_GET[QUETAG]; 
			
			//print_r($wp_query->query_vars);	
				if ( in_array ( $wp_query->query_vars[QUETAG], array_keys ( $this->show_page_on_front_array ) ) && '' != $wp_query->query_vars['page_id'] ) {	
					$curlang = $wp_query->query_vars[QUETAG];
				 
					//error_log( '-----GET------'. $curlang );
				} elseif ( in_array ( $wp_query->query_vars[QUETAG], array_keys ( $this->show_page_on_front_array ) ) ){
			 	...
			 	$trans_id = $this->linked_post_in( $original_id, $curlang ) ; //echo get_post_meta($original_id, 'lang-'.$curlang, true);
				//error_log( $curlang." ----".$original_id.'==== '.$trans_id ) ;
				if ( '' != $trans_id ) { 
					return $trans_id;
				} else {
					return $original_id;	
	...
	// Is the user sufficiently leveled, or has the bar been disabled? !is_super_admin() || 
	 ...
	 // editor rights
		if ( current_user_can ( 'xili_language_menu' ) )
			add_action('admin_bar_menu', array(&$this,'xili_admin_bar_links') , 500 );
	...
	function xili_admin_bar_links() {
		...
		if ( class_exists('xili_dictionary' ) && current_user_can ('xili_dictionary_set')) 
			$wp_admin_bar->add_menu( array(
				...
				'href' => admin_url( 'tools.php?page=dictionary_page' ),
				'parent' => 'xili_links',
	...
	function xili_language_checkboxes() { 
		...
		if ($listlanguages) { //1.5.5
			...
			?> 
			<label style="border:solid 1px grey; margin:1px 0px; padding:3px 2px; width:78px; display:inline-block; " for="xili_language_check_<?php echo $language->slug ; ?>" class="selectit"><input id="xili_language_check_<?php echo $language->slug ; ?>" name="xili_language_set" type="radio" value="<?php echo $language->slug ; ?>"  <?php if($curlangname==$language->name) echo 'checked="checked"' ?> />&nbsp;<?php echo _e($language->description, 'xili-language'); ?></label>
			<?php  
			} /*link to top of sidebar*/?> 
			<br /><label style="border:solid 1px grey; margin:1px 0px; padding:3px 2px; width:165px; display:inline-block;" for="xili_language_check" class="selectit"><input id="xili_language_check" name="xili_language_set" type="radio" value="" <?php if($curlangname=="") echo 'checked="checked"' ?> />&nbsp;<?php _e('undefined','xili-language') ?></label><br /></div>
	...
	function xili_language_linked_posts() { 
		...
		<thead>
		<tr ><th><?php _e('Language','xili-language'); ?></th><th>&nbsp;</th><th align="center"><?php _e($theid,'xili-language'); ?></th><th align="left"><?php _e('Display','xili-language'); ?></th><th align="left"><?php _e('Edit'); ?></th></tr>
		</thead>
		...
		if ($test === true) $autosearch = get_post_meta($post_ID,'_xl-search-linked',true);
			foreach ($listlanguages as $language) {
				...
				$i = 1 - $i ;
				$bkcolor = ( $i == 0 ) ? "transparent":"#e0e0e0";
				?>
				<tr style="background-color:<?php echo $bkcolor ?>;"><th>
				<label for="xili_language_<?php echo QUETAG.'-'.$language->slug ; ?>"><?php _e($language->description,'xili-language') ; ?></label></th><td>&nbsp;<?php
				if ( $post_type == 'page' || $post_type == 'post' ) { 
				?><a class="hide-if-no-js" onclick="findPosts.open( 'lang[]','<?php echo $language->slug ?>' );return false;" href="#the-list" title="<?php _e( 'Search linked post', 'xili-language' ); ?>"><?php _e( '?', 'xili-language' ); ?></a>
				<?php }
				?></td><td align="center"><input id="xili_language_<?php echo QUETAG.'-'.$language->slug ; ?>" name="xili_language_<?php echo QUETAG.'-'.$language->slug ; ?>"  value="<?php echo $otherpost; ?>" style="width:40px" /><input type="hidden" name="xili_language_rec_<?php echo QUETAG.'-'.$language->slug ; ?>" value="<?php echo $otherpostr; ?>"/>
			...
			if ($test === true && "" != $postlang) { 
				...
				$bkcolor = ( $i == 0 ) ? "transparent":"#e0e0e0";
			?>	
			<tr style="background-color:<?php echo $bkcolor ?>;"><th><?php _e('Check','xili-language'); ?></th><td><input id="xili_language_search_lang" name="xili_language_search_lang" type="checkbox" value="searchlinked" <?php if('' != $autosearch) echo 'checked="checked"' ?> /></td><td colspan = 3 ><small><?php echo$autosearchmess ; ?></small></td></tr>
			<?php } ?>
				</tbody></table>
...
//echo '->'.$id.$langmeta.$targetID.'</br>'; 
	...
	function languages_settings() { 
		...
		switch( $action ) {
			...
			case 'updateoptions'; // sidebox 3 - below in source
				...
				if ( $_POST['force_permalinks_flush'] == 'enable' ) {
	...
	function  on_sidebox_3_content( $data ) { 
		...
		if ($this->lang_perma ) { // to force permalinks flush ?> 
			<br /><fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo _e('Permalinks rules', 'xili-language'); ?></legend>
