...
define('XILILANGUAGE_VER','1.7.1'); /* used in admin UI*/
...
class xili_language {
	...
	function xili_language($metabox = false, $post_ajax = false, $locale_method = false) {
		...
		//$this->add_action('xili_language_list_options','xili_language_list_options',10,1);
	}
	...
	function xiliml_override_load ($falseval, $domain, $mofile) {
	   if (false === strpos ($mofile ,'wx_YZ.mo')) {//
	   		...
	   		$this->xili_settings['langs_folder'] = str_replace('/wx_YZ.mo','',str_replace(get_template_directory(),'',$mofile));
	   		return true;
	   }	
	}
...
//echo '->'.$id.$langmeta.$targetID.'</br>'; 
	...
	function languages_settings() { 
		...
		switch($action) {
			...
			case 'menuadditems';	
				check_admin_referer( 'xilimloptions' );
				$menuid = $_POST['wpmenu_id'];
				// function below sidebox_4
				$result = $this->add_list_of_language_links_in_wp_menu($menuid);
		...
		$data = array('message'=>$message, 'action'=>$action, 'formtitle'=>$formtitle, 'language'=>$language,'submit_text'=>$submit_text,'cancel_text'=>$cancel_text,'browseroption'=>$this->browseroption, 'authorbrowseroption'=>$this->authorbrowseroption , 'functions_enable'=>$this->functions_enable, 
	...
	function  on_sidebox_4_content($data) { 
		extract($data);
	 	...
	 	<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo __("Nav menu: Home links in each language",'xili-language'); ?></legend>
	 		<?php $menu =  get_nav_menu_locations(); $menuid = 0;
		 	if (is_array($menu)) { $menuslug =  array_keys($menu); $menuid = $menu[$menuslug[0]]; }
		 	if ( $menuid != 0 ) {
		 	?>
	 		<label for="list_in_nav_enable" class="selectit"><input id="list_in_nav_enable" name="list_in_nav_enable" type="checkbox" value="enable"  <?php if($list_in_nav_enable =='enable') echo 'checked="checked"' ?> /> <?php echo _e('Add list in nav menu', 'xili-language'); ?></label><br /><br />
	 	...
	 	<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo __("Theme's nav menu items settings:",'xili-language'); ?></legend>
		 	...
		 	printf (__("This theme (%s) contains \"%s\" Nav Menu.",'xili-language'), get_option("template"), $menuitem->name );
		 	
		 	
		 	$items = get_objects_in_term( $menuitem->term_id, 'nav_menu' );
		 	echo '<br />'; 
		 	printf (__("This menu has %s item(s);",'xili-language'), count($items));
			...
			<div class="submit"><input  id='menuadditems' name='menuadditems' type='submit' value="<?php _e('Add menu items','xili-language') ?>" /></div>
		 	<input type="hidden" id="wpmenu_id" name="wpmenu_id" value="<?php echo $menuid ?>" />
		 	<?php } else {
	 ...
	 *
	 */
	function add_list_of_language_links_in_wp_menu ($menuid) {
		$defaultarray = array(
	...
	function xili_language_list($before = '<li>', $after ='</li>', $option='', $echo = true) {
		...
		if ($option == 'typeone') {
			...
			foreach ($listlanguages as $language) {
				...
				$a .= $beforee ."<a href='".get_bloginfo('url')."/?".QUETAG."=".$language->slug."' title='".__('Posts selected',$this->thetextdomain)." ".__('in '.$language->description,$this->thetextdomain)."'>". __('in '.$language->description,$this->thetextdomain) ."</a>".$after;
			}
			...
			foreach ($listlanguages as $language) {
				...
				$a .= $before ."<a{$class} href='".get_bloginfo('url')."/?".QUETAG."=".$language->slug."' title='".__('Posts selected',$this->thetextdomain)." ".__('in '.$language->description,$this->thetextdomain)."'>". __('in '.$language->description,$this->thetextdomain) ."</a>".$after;
			}
		}
		if ($echo) 
				echo $a;
	...
	*
	...
	function xl_get_the_category_list( $thelist, $separator = '', $parents='') {
		...
		if ( empty( $categories ) )
			return apply_filters( 'the_category', __( 'Uncategorized',$this->thetextdomain ), $separator, $parents );
...
function xiliml_php2loc_time_format_translator ($phpformat = 'm/d/Y H:i') {
	...
	$phpformchar = array('A' ,'a' ,'D' ,'l' ,'g' ,'d' ,'j' ,'z' ,'N' ,'w ','W' ,'M' ,'F' ,'h' ,'M ','m' ,'y' ,'Y' ,'H' ,'G' ,'i' ,'S' ,'s' ,'O');
	/* doc here: http://fr2.php.net/manual/en/function.date.php */
	$locformchar = array('%p','%P','%a','%A','%l','%d','%e','%j','%U','%w','%W','%b','%B','%I', '%h', '%m','%y','%Y','%H','%l','%M', '','%S','%z');
