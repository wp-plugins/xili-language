...
License: GPLv2
*/
# updated 111220 - 2.3.9 - if needed and present add rtl.css for rtl languages - new date - time options
...
define('XILILANGUAGE_VER','2.3.9'); /* used in admin UI*/
...
class xili_language {
	...
	function xili_language( $locale_method = false, $show = false ) {
		...
		if( false === $this->xili_settings ) { //1.9.1
			...
			if ( ! isset ( $this->xili_settings['version'] ) || $this->xili_settings['version'] != '2.0') { // repair
				$this->initial_settings ();
				update_option('xili_language_settings', $this->xili_settings);
			}
		}
	...
	function initial_settings () {
		$this->xili_settings = array(
				...
				'wp_locale' => 'db_locale' // 2.4.0 = old mode
		    );
	}
	...
	function languages_settings() { 
		...
		$themessages[9] = __('Email not sent. Please verify email field','xili-language');
	...
	function  on_sidebox_3_content( $data ) { 
		...
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo _e('Locale (date) translation', 'xili-language'); ?></legend>
			<?php echo _e("Since v2.4, new way for locale (wp_locale) translation.", "xili-language"); ?><br /><br />
			<label for="xili_language_wp_locale"><?php _e('Mode wp_locale','xili-language') ?> <input id="xili_language_wp_locale" name="xili_language_wp_locale" type="checkbox" value="wp_locale" <?php if( $this->xili_settings['wp_locale'] == 'wp_locale') echo 'checked="checked"' ?> /></label>
		</fieldset>
