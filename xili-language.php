...
define('XILILANGUAGE_VER','0.9.9.1'); /* used in admin UI*/
...
class xili_language {
	...
	function languages_settings() { 
		...
		switch($action) {
			case 'updateoptions';
				...
				update_option('xili_language_settings', $this->xili_settings);
				$message .= " - ".__('Option is updated.','xili-language')." (=> ".$this->browseroption.") (".$this->authorbrowseroption.")";
				$actiontype = "add";
				break;
			case 'add':
		...
		$data = array('message'=>$message,'messagepost'=>$messagepost,'action'=>$action, 'formtitle'=>$formtitle, 'language'=>$language,'submit_text'=>$submit_text,'cancel_text'=>$cancel_text,'browseroption'=>$this->browseroption, 'authorbrowseroption'=>$this->authorbrowseroption);
		?>
	...
	function xili_language_list($before = '<li>', $after ='</li>',$option='') {
			/* default here*/
		//$listlanguages = get_terms(TAXONAME, array('hide_empty' => false)); - now sorted from group -
		$listlanguages = get_terms_of_groups_lite ($this->langs_group_id,TAXOLANGSGROUP,TAXONAME,'ASC');
		foreach ($listlanguages as $language) {
