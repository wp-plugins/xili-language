...
define('XILILANGUAGE_VER', '2.12.0'); /* used in admin UI*/
...
class xili_language {
	...
	function posts_join_with_lang( $join, $query_object = null ) {
		...
		xili_xl_error_log ( '===== ready_to_join_filter == 1363 =======' . serialize($this->ready_to_join_filter));
		xili_xl_error_log ( '===== posts_join_with_lang == 1364 =======' . serialize($query_object->query_vars));
		...
		if ( isset ( $query_object->query_vars[QUETAG] ) && '' != $query_object->query_vars[QUETAG] ) {
				...
				xili_xl_error_log ('before lang_perma (insert_join) *** #1377 *** ' . serialize($insert_join));
		...
		xili_xl_error_log ('ici - join (#1398) = ' . serialize($insert_join) );
		xili_xl_error_log ('ici - join (#1399 - post_type) = ' . serialize($query_object->query_vars['post_type']) );
	...
	function posts_where_lang( $where, $query_object = null ) {
		...
		xili_xl_error_log ( "****** where start *** (#1453) ***".$where );
		if ( isset ( $query_object->query_vars[QUETAG] )) { xili_xl_error_log( 'where #1454 = ' . $query_object->query_vars[QUETAG] );}
		...
		if ( "" != $this->sublang ) { // see above
			...
			xili_xl_error_log ('ici - doit where (#1503) = ' . serialize($do_it) );
			...
			if ( $do_it ) { // insertion of selection
				...
				if ( strpos($query_object->query_vars[QUETAG], ',') !== false ) {
					...
					if ( ! isset( $query_object->query_vars['post_format'] ) ) {
						/* only one lang */
						xili_xl_error_log ( $query_object->query_vars['page_id'] . ' <-----+++**** 1527 where blog page_for_posts ' . $query_object->query_vars[QUETAG] . '-' . get_option('page_on_front'));
						$need_join = true;
						$where .= " AND xtt.taxonomy = '".TAXONAME."' ";
				...
				if ( $query_object->is_home && $this->show_page_on_front ) {
					...
					xili_xl_error_log ( $query_object->query_vars['page_id'] . ' <-----+++**** 1549 where home ' . $query_object->query_vars[QUETAG] . '-' . get_option('page_on_front'));
				}
				...
				if ( ($query_object->is_home && !$this->show_page_on_front && $this->xili_settings['homelang'] == 'modify') || ($query_object->is_home && $query_object->is_posts_page && $this->xili_settings['pforp_select'] != 'no_select' ) ) {
					...
					if ( $query_object->is_posts_page ) { // 2.8.4
						...
						if ( $this->is_permalink ) { // 2.8.4.1
							xili_xl_error_log ( "****** ici 1598 force change ******" . serialize ( $query_object->query_vars['pagename']) );
							...
							if ( isset ( $query_object->query_vars['pagename'] ) && isset ( $pagenametolang[$query_object->query_vars['pagename']] ) ) {
								...
								xili_xl_error_log ( '"****** ici 1602 Name + ' . $curlang );
							} else {
							...
							xili_xl_error_log ( '****** ici 1610 force change ****** ' . serialize ( $query_object->query_vars['page_id']) );
		...
		xili_xl_error_log ( "****** where *** (#1645) ***".$where );
