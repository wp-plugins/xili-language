...
# updated 120429 - 2.6.0 - class admin in separate file, able to detect and use local files (local-xx_XX.mo) containing translations
...
define('XILILANGUAGE_VER','2.6-rc1'); /* used in admin UI*/
...
class xili_language {
	...
	public function __construct( $locale_method = false, $show = false, $class_admin = false ) {
		...
		if( false === $this->xili_settings ) { //1.9.1
			...
			if ($this->xili_settings['version'] == '2.1') { /* 2.6.0 */	
				$this->xili_settings['external_xl_style'] = "off";
		...
		if ( ! $class_admin ) add_action( 'init', array(&$this,'init_and_register_language_post_taxonomy') ); // only one time
	...
	function initial_settings () {
		$this->xili_settings = array(
				...
				'creation_redirect' => '', // 2.6 to redirect to new post after creation
				'external_xl_style' => "off" // activate external xl-style.css
		    );
	}
