...
define('XILILANGUAGE_VER','1.5.5'); /* used in admin UI*/
...
class xili_language {
	...
	/**
	 * Add filters of texts of comment form  
	 *
	 * @since 1.5.5
	...
	function xili_comment_form_default_fields ($fields) {
		...
		$fields =  array(
			'author' => '<p class="comment-form-author">' . '<label for="author">' . __( 'Name',$this->thetextdomain ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
			            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></p>',
			'email'  => '<p class="comment-form-email"><label for="email">' . __( 'Email',$this->thetextdomain ) . '</label> ' . ( $req ? '<span class="required">*</span>' : '' ) .
			            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></p>',
			'url'    => '<p class="comment-form-url"><label for="url">' . __( 'Website',$this->thetextdomain ) . '</label>' .
	...
	function xili_comment_form_defaults ($defaults) {
		...
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( 'Comment', $this->thetextdomain ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>',
		'must_log_in'          => '<p class="must-log-in">' .  sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.',$this->thetextdomain ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>',$this->thetextdomain ), admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes">' . __( 'Your email address will not be published.',$this->thetextdomain ) . ( $req ? __( ' Required fields are marked <span class="required">*</span>',$this->thetextdomain ) : '' ) . '</p>',
		'comment_notes_after'  => '<dl class="form-allowed-tags"><dt>' . __( 'You may use these <abbr title="HyperText Markup Language">HTML</abbr> tags and attributes:',$this->thetextdomain ) . '</dt> <dd><code>' . allowed_tags() . '</code></dd></dl>',
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
		'title_reply'          => __( 'Leave a Reply',$this->thetextdomain ),
		'title_reply_to'       => __( 'Leave a Reply to %s',$this->thetextdomain ),
		'cancel_reply_link'    => __( 'Cancel reply',$this->thetextdomain ),
		'label_submit'         => __( 'Post Comment',$this->thetextdomain ),
		);
	 ...
	 *
	 * @since 1.3.0
	 * @updated 1.3.1 - Add New specific for pages - 1.5.5 - new post and post_type=page
	 *
	 */
	function newlinkedpost($postlang = "" , $targetlang = "") {
		...
		if ($this->notwp3) {
			...
			$whatnew = 'post';
			$type = ($post->post_type == 'post') ? '' : 'post_type=page&';
		}	
		...
		if ($option == 'typeone') {
			...
			foreach ($listlanguages as $language) {
				if ($language->slug != $this->curlang ) {
					...
					$a .= $before ."<a{$class} href='".$currenturl.QUETAG."=".$language->slug."' title='".__('Posts selected',$this->thetextdomain)." ".__('in '.$language->description,$this->thetextdomain)."'>". __('in '.$language->description,$this->thetextdomain) ."</a>".$after;
				}
			}
			echo $a;
		} else {	/* current list */
			foreach ($listlanguages as $language) {
				...
				$a .= $before ."<a{$class} href='".get_bloginfo('siteurl')."/?".QUETAG."=".$language->slug."' title='".__('Posts selected',$this->thetextdomain)." ".__('in '.$language->description,$this->thetextdomain)."'>". __('in '.$language->description,$this->thetextdomain) ."</a>".$after;
			}
			echo $a;
		}	
	}
