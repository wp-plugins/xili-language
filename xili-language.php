...
define('XILILANGUAGE_VER','1.3.0'); /* used in admin UI*/
...
class xili_language {
	 ...
	 * @updated 1.3.0 to avoid delete relationship when trashing
	 * @param $post_ID
	 */
	...
	function xili_language_linked_posts() { 
		...
		if(0 != $post_ID){
			...
			if ('' != $obj_term->slug) :
				...
				$postlang = ""; /* when created before plugin */
			endif;
			...
			foreach ($listlanguages as $language) {
				...
				if ($line) {
					if (0 != $post_ID) {
						...
						$otherpostr = $otherpost; /* since 1.3.0 */
					} else { /* since 1.3.0 */
	...
	function newlinkedpost($postlang = "" , $targetlang = "") {
		...
		if ($post->post_status == 'publish' || $post->post_status == 'pending' || $post->post_status == 'draft') {
			...
			return "</td><td><small>".__('*','xili-language')."</small></td><td><a href='post-new.php?xlfrom_id=".$post->ID."&xlfrom_lang=".$postlang."&xltgt_lang=".$targetlang."' target='_blank' >".__('Add New')."</a></td><tr>";
			}
		} else {
			return "</td></tr>";
		}
