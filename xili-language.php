...
define('XILILANGUAGE_VER','2.1.9'); /* used in admin UI*/
		...
		if ( '*' == $query_object->query_vars[QUETAG] ) { // undefined 
			...
			if ($query_object->query_vars['caller_get_posts']) { 
				...
				if ( $this->lang_perma ) {
					...
					if ( $query_object->is_tax &&  $query_object->query_vars['taxonomy'] == 'category' ) $do_it = true;
					if ( $query_object->is_tax &&  $query_object->query_vars['taxonomy'] == TAXONAME && "" == $query_object->query_vars['category_name'] ) $do_it = false;
					//error_log ( '-----perma--where--lang---'. (int)$do_it );
				} else {
		...
		if ( have_posts() ) {
			if ( !is_front_page() ) { /* every pages */ 
				...
				if ( is_page() ) {
					...
					// if ( isset( $_GET[QUETAG] ) ) $curlang = $_GET[QUETAG]; /*useful when no result*/
...
//echo '->'.$id.$langmeta.$targetID.'</br>'; 
	...
	function xiliml_langinsearchform ( $before='', $after='', $echo = true ) {
			...
			foreach ($listlanguages as $language) {
				...
				$selected = ( $language->slug == $this->curlang ) ? 'checked="checked"' : "" ; //2.2.0
				$a .= $before.'<input type="radio" name="'.QUETAG.'" value="'.$language->slug.'" id="'.$language->slug.'" '.$selected.' />&nbsp;'.__( $language->description, $this->thetextdomain ).' '.$after;
			}			
