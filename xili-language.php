...
define('XILILANGUAGE_VER','1.8.5'); /* used in admin UI*/
...
class xili_language {
	...
	var $domaindetectmsg = "";
	var $langs_list_options = array(array('','default'),array('typeone','Type n°1')); // type of languages list see options in xili_language_list
	...
	function xili_language($locale_method = false) {
		global $wp_version;
	...
	function xili_add_pages() {
		 $this->thehook = add_options_page(__('xili-language plugin','xili-language'), __('Languages','xili-language'), 'manage_options', 'language_page', array(&$this,'languages_settings'));
	...
	function on_load_page() {
			...
			add_meta_box('xili-language-sidebox-mail', __('Mail & Support','xili-language'), array(&$this,'on_sidebox_mail_content'), $this->thehook , 'normal', 'core');
	}
...
//echo '->'.$id.$langmeta.$targetID.'</br>'; 
		...
		switch( $action ) {
			...
			case 'sendmail'; // 1.8.5
				...
				$headers = 'From: Xili-PostinPost Page <' . get_bloginfo ('admin_email').'>' . "\r\n" ;
	   			...
	   			$message .= "\n\n"; 
	   			$result = wp_mail('contact@xiligroup.com', $_POST['thema'].' from xili-language plugin settings page.' , $message, $headers );
	...
	function  on_sidebox_4_content( $data ) { 
		...
		if ( function_exists('is_child_theme') && is_child_theme() ) { // 1.8.1 and WP 3.0
	 	...
	 	if ( $this->ltd === false )	echo '<br /><span style="color:red">'.__('CAUTION: no load_textdomain() in functions.php - review the content of file in the current theme.','xili-language').'</span>'; 
	...
	function link_language_meta_box( $link) {
		...
		echo '<h4>'.__('Check the language for this link','xili-language').'</h4>';
		...
		foreach ($listlanguages as $language) { ?> 
				...
				<?php } /*link to top of sidebar*/?> 
				<label for="xili_language_check" class="selectit"><input id="xili_language_check_ever" name="xili_language_set" type="radio" value="ev_er" <?php if($curlangname=="ev_er") echo 'checked="checked"' ?> /> <?php _e('Ever','xili-language') ?></label>
				<label for="xili_language_check" class="selectit"><input id="xili_language_check" name="xili_language_set" type="radio" value="" <?php if($curlangname=="") echo 'checked="checked"' ?> /> <?php _e('undefined','xili-language') ?></label><br />
			  	<br /><small>© xili-language <?php echo XILILANGUAGE_VER; ?></small>
		<?php
 ...
 */
$xili_language =& new xili_language(false); 
