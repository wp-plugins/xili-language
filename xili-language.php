...
define('XILILANGUAGE_VER','2.3.1'); /* used in admin UI*/
...
class xili_language {
	...
	function xili_language( $locale_method = false, $show = false ) {
		...
		if( false === $this->xili_settings ) { //1.9.1
			...
			if ( ! isset ( $this->xili_settings['version'] ) || $this->xili_settings['version'] != '1.9') { // repair
				$this->initial_settings ();
				update_option('xili_language_settings', $this->xili_settings);
			}
		}
		...
		if ( !is_admin() ) add_filter( 'language_attributes',  array(&$this,'head_language_attributes'));
		...
		if ( isset ( $query_object->query_vars[QUETAG] ) && '' != $query_object->query_vars[QUETAG] ) {
			if ( ( isset ( $query_object->query_vars['caller_get_posts'] ) && $query_object->query_vars['caller_get_posts']) || ( isset ( $query_object->query_vars['ignore_sticky_posts'] ) && $query_object->query_vars['ignore_sticky_posts']) ) {
				...
				if ( $this->lang_perma ) { 
					...
					// && $query_object->query_vars['taxonomy'] == TAXONAME && $query_object->is_tax
	...
	function posts_search_filter ( $search, $the_query) {
		...
		if ( isset( $the_query->query_vars[QUETAG] ) && false !== strpos( $the_query->query_vars[QUETAG] , LANG_UNDEF ) ) {
			...
			//array_shift ($the_query->tax_query->queries ); ? if not first ?
			if ( array() != $the_query->tax_query->queries ) {
				...
				$the_query->tax_query->queries = $new_queries; //print_r($the_query->query_vars);
			}
	...
	function xili_language_linked_posts() { 
		...
		$i = 0;
		?>
			<p><em><?php printf(__('ID of %s in other languages:','xili-language'), $post_type_object->label); ?></em></p>
		<?php $theid = $post_type.' ID';
			$autosearchmess = sprintf(__('to auto search linked %s. (read docs)','xili-language'), $post_type_object->label) ;
		?>
	...
	function languages_settings() { 
				...
				if ( current_theme_supports( 'menus' ) ) {
					...
					if ( $menu_locations ) { 
						foreach ($menu_locations as $menu_location => $location_id) {						
							if ( $_POST['xili_navmenu_check_option_'.$menu_location] == 'enable' ) {
		...
		$data = array(
			'message'=>$message, 'action'=>$action, 'formtitle'=>$formtitle, 'language'=>$language,'submit_text'=>$submit_text,'cancel_text'=>$cancel_text,'browseroption'=>$this->browseroption, 'authorbrowseroption'=>$this->authorbrowseroption , 'functions_enable'=>$this->functions_enable,
