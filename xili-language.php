...
*/
# 2.8.9 - 130527 - fixes, __construct in widget classes.
...
define('XILILANGUAGE_VER', '2.8.9'); /* used in admin UI*/
...
define('XILILANGUAGE_DEBUG', false ); /* used in dev step UI see #4516 - xili_xl_error_log () */
...
class xili_language {
	...
	function only_parent_construct () {	
		...
		add_action( 'plugins_loaded', array(&$this,'init_plugin_textdomain'), 9 ); // 2.8.9 - class for theme-xili
	 ...
	 */
	function initial_settings() {  xili_xl_error_log ( '# 422 -------------------- init' );
		return array(
			    'taxonomy'		=> 'language',
			    'version' 		=> '2.5',
	...
	function posts_join_with_lang( $join, $query_object = null ) {
		...
		if ( isset ( $query_object->query_vars[QUETAG] ) && '' != $query_object->query_vars[QUETAG] ) { 
			xili_xl_error_log( 'where #1193 = ' . $query_object->query_vars[QUETAG] );
	...
	function posts_where_lang( $where, $query_object = null ) {
		...
		if ( isset ( $query_object->query_vars[QUETAG] )) xili_xl_error_log( 'where #1297 = ' . $query_object->query_vars[QUETAG] );
		...
		if ( "" != $this->sublang )  { // see above
			...
			if ($do_it) { // insertion of selection
				...
				if ( $query_object->is_home && $this->show_page_on_front ) {  
					...
					xili_xl_error_log ( $query_object->query_vars['page_id'] . ' <-----+++**** 1387 where home ' .  $query_object->query_vars[QUETAG] . '-' . get_option('page_on_front'));
				}
	...
	function change_plugin_domain ($translation, $text, $domain ) {
		...
		xili_xl_error_log ( 'Line #1772 - domain = ' . $domain . ' -> ' . $text . ' : ' . $translations->translate( $text ) );
	...
	function get_curlang_action_wp () {
		...
		if ( have_posts() ) {
			...
			if ( $condition ) { /* every pages !is_front_page() */ 
				...
				if ( isset ( $wp_query->query_vars[QUETAG] ) && '' != $wp_query->query_vars[QUETAG] ) { 
					...
					xili_xl_error_log('# 1889 ---->>>'.$curlang );
				} else {
		...
		xili_xl_error_log ('#1916 - fin wp=' . $curlang );
...
function xili_jetpack_lang_reload ( $locale = 'en_US', $domain = 'default' ) { 
	...
	if ( $domain == 'xili-language' && class_exists ( 'jetpack' ) )  {  // because plugin jetpack domain not filterable
		...
		unload_textdomain( 'jetpack' ); // by Jetpack_Sync::sync_options( __FILE__, 'widget_twitter' ) during plugin loading
