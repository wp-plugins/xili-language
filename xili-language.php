...
*/
# updated 130422 - 130506 - 2.8.8 - fixes notice with bbPress 2.3 - try to find .mo files at automattic svn and GlotPress - best title in href language list - new way to insert languages list in navigation menu - better filter in menu/widget title
...
define('XILILANGUAGE_VER', '2.8.8-b3'); /* used in admin UI*/
	...
	function set_mofile( $curlang ) {
		...
		if ( '' != $filename ) {
			...
			if ( $this->xili_settings['mo_parent_child_merging'] ) {
				...
				//error_log ( serialize ( $parent_load ) .  $mofile );
			}
