...
the name of plugin  was xili-language widget
now it is a part of xili-language plugin. 
...
class xili_Widget_Recent_Posts extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if (class_exists('xili_language')) {
			...
			if ($the_lang == '') 
				$thequery = array('showposts' => $number, 'nopaging' => 0,
					'post_type' => $post_type_arr , 
					'post_status' => 'publish', 'caller_get_posts' => 1);
			else if ($the_lang == '*')	
			 	$thequery = array ('xlrp' => 1, 'showposts' => $number, 'nopaging' => 0,
			 		'post_type' => $post_type_arr ,
			 	 	'post_status' => 'publish',  'caller_get_posts' => 1, QUETAG => the_curlang()); 
			else
				$thequery = array ('xlrp' => 1, 'showposts' => $number, 'nopaging' => 0,
					'post_type' => $post_type_arr ,
					'post_status' => 'publish',  'caller_get_posts' => 1, QUETAG => $the_lang); 	
			//echo '===='.the_curlang();
