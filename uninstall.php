...
function delete_taxonomies ($xl_settings) {
	...
	foreach ($languages as $language )  {
		...
		// delete terms
		$linklang = is_term($language->slug,'link_'.$xl_settings['taxonomy']);
			if ( $linklang ) wp_delete_term( $term_id, 'link_'.$xl_settings['taxonomy'] );
		wp_delete_term( $term_id, $xl_settings['taxonomy'] );
	 
	} 
	$term_group = is_term( 'ev_er', 'link_'.$xl_settings['taxonomy'] ); /* special ever language for links */
	// link_language links
	...
	$term_group = is_term( 'the-langs-group', $xl_settings['taxolangsgroup'] );
