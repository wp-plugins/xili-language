...
define('XILILANGUAGE_VER','1.1.es'); /* used in admin UI*/
...
class xili_language {
	...
	function  on_sidebox_4_content() { 
	 	$update_nonce = wp_create_nonce('xilimloptions');
	 	?>
		<p><?php _e("Special Actions",'xili-language') ?></p>
		<?php
		if (functions_enable !='' && function_exists('xiliml_setlang_of_undefined_posts')) {
		?>
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo _e('Update previous undefined posts', 'xili-language'); ?></legend>
		<select name="xili_language_toset" id="xili_language_toset" style="width:100%;">
				<option value="" ><?php _e('Choose target language…','xili-language'); ?></option>
				<?php $listlanguages = get_terms_of_groups_lite ($this->langs_group_id,TAXOLANGSGROUP,TAXONAME,'ASC');
			foreach ($listlanguages as $language) {
			...
			<label for="from_categories"><?php _e('Categories') ?></label>:&nbsp;<input name="from_categories" id="from_categories" type="text" value="1" size="16" />
			<div class='submit'>
		<input id='updateundefined' name='updateundefined' type='submit' tabindex='6' value="<?php _e('Set undefined posts...','xili-language') ?>" /></div>
		<?php echo wp_nonce_field( 'xilimloptions', '_ajax_nonce', true, false );/**/ ?>
		</fieldset>
		<?php }
	}
