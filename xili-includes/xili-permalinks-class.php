...
class XL_Permalinks_rules {
	...
	function __construct( ) {
		...
		add_filter ( 'post_type_link', array(&$this, 'insert_lang_4post'), 10, 3 );
	...
	function type_rewrite_rules ( $rules ) {
		...
		xili_xl_error_log ( '******************>>> ' . serialize ( $newrules ));
 ...
 *
	...
	function insert_lang_tag_root ( $url, $path, $orig_scheme, $blog_id ) {
		...
		$enabled_custom_posts = array_keys ( $xili_language->xili_settings['multilingual_custom_post'] ) ;
	...
	function insert_lang_taxonomy ( $termlink, $term, $taxonomy ) {
		...
		if ( in_array ( $taxonomy , array ( 'category', 'post_format' ) ) ) {
		...
		if ( in_array ( $taxonomy , array ( 'language' ) ) ) {
