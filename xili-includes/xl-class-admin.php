	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %s is ready for 4.1 “Dinah” new install with twenty fifteen theme. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';

				$pointer_text .= '<p>' . esc_js( sprintf( __( 'Previous version of %s improves compatibility with WP 4.0 when starting a very new website, improves new way/option to use flag of media asset and default from theme subfolder. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
