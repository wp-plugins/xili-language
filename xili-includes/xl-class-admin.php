...
class xili_language_admin extends xili_language {
	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %s improves plugin textdomain and mo file switching (more rules). See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
...
// $this->xili_settings['functions_enable']
	...
	function on_sidebox_for_specials ( $data ) {
		...
		<fieldset class="box"><legend><?php _e('Translation domains settings', 'xili-language'); ?></legend><p>
			...
			<?php
			echo '<tr><th style="width:25%">' . __('Name', 'xili-language') . '</th><th style="width:25%" >' . __('Rule', 'xili-language') . '</th><th style="width:20%">' . __('Plugin Domain', 'xili-language') . '</th><th style="width:20%">' . __('Domain Path', 'xili-language') . '</th></tr>';
			echo '</thead><tbody>';
