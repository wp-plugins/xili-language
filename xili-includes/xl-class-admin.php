...
class xili_language_admin extends xili_language {
	...
	function xili_tool_bar_links() {
		...
		$link = plugins_url( 'images/xililang-logo.gif', $this->file_file ) ;
	...
	function more_plugin_row ( $plugin_file, $plugin_data, $status ) {
		...
		if ( $plugin_file == $base ) {
			...
			$cb_col = '<img src="'.plugins_url( 'images/xililang-logo.gif', $this->file_file ).'" alt="xili-language trilogy"/>';
		...
		<div id="poststuff" <?php echo $poststuff_class; ?>>
			<div id="post-body" <?php echo $postbody_class; ?> >
				...
				<div id="post-body-content">
					...
					<h4><a href="http://dev.xiligroup.com/xili-language" title="Plugin page and docs" target="_blank" style="text-decoration:none" ><img style="vertical-align:middle" src="<?php echo plugins_url( 'images/xililang-logo-32.png', $this->file_file ) ;  ?>" alt="xili-language logo"/>  xili-language</a> - © <a href="http://dev.xiligroup.com" target="_blank" title="<?php _e('Author'); ?>" >xiligroup.com</a>™ - msc 2007-2012 - v. <?php echo XILILANGUAGE_VER; ?></h4>	
	...
	function on_sidebox_for_specials ( $data ) {
		...
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo _e('Dashboard style: External xl-style.css', 'xili-language'); ?></legend>
		<?php
		...
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo _e('Locale (date) translation', 'xili-language'); ?></legend><p>
	 	...
	 	<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo _e('Redirect to created post', 'xili-language'); ?></legend><p>
			<?php echo _e("After creating a linked post in other language, the Edit post is automatically displayed.", "xili-language"); ?><br /><br />
	...
	function on_sidebox_4_theme_info( $data ) {
		...
		if ( function_exists('is_child_theme') && is_child_theme() ) { // 1.8.1 and WP 3.0
	 	...
	 	if ( $this->parent->ltd === false )	echo '<br /><span style="color:red">'.__('CAUTION: no load_theme_textdomain() in functions.php - review the content of file in the current theme or choose another canonical theme.','xili-language').'</span>'; 
...
<?php
	...
	function post_status_addons ( $post_ID, $curlang ) {
		...
		?>
		<p><?php echo $refresh; ?>
		<?php if ( '' != $curlang && current_user_can ('xili_language_clone_tax') ) {
	 ...
	 */
	 ...
	 //	echo '.postsbody { border:1px solid black; }';
	 	...
	 	if ( $lang != ''  && file_exists( $this->style_flag_folder_path . $lang .'.png' ) )
	 	...
	 	$listlanguages = $this->get_listlanguages();
	 	foreach ($listlanguages as $language)  {	
 			if ( file_exists( $this->style_flag_folder_path . $language->slug .'.png' ) && $this->xili_settings['external_xl_style'] == "on" ) {
	 ...
	 function add_help_text( $contextual_help, $screen_id, $screen ) { 
	  	if ( in_array ( $screen->id , array ('settings_page_language_page', 'settings_page_language_front_set',  'settings_page_language_expert','settings_page_language_support') ) ) {
	  		...
	  		$wiki_page[ 'settings_page_language_support' ] = '/index.php/Xili-language_settings:_Assistance,_support_form';
	   		...
	   		$contextual_help =
	   	  '<p>' . sprintf( __('About this tab %s:','xili-language'), $page_title[$screen->id] ) . '</p>' .
	   	  '<ul>' . 
