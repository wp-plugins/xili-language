	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %s incorporate direct link in language list compatible with new language grouping of xili-tidy-tags plugin for Archives Tags webpage.', 'xili-language' ) , XILILANGUAGE_VER) ). '</p>';
		...
		if ( current_theme_supports( 'menus' ) ) { ?>
	 	...
	 	<fieldset class="box"><legend><?php echo __("Nav menu: Home links in each language",'xili-language'); ?></legend>
			...
			foreach ( $menu_locations as $menu_location => $location_id ) { 
				...
				<?php
				$point = $this->has_languages_list_menu ( $menu_location );
