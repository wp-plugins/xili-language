	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __('This version %s starts to test WP 3.9rc. See also %s.','xili-language') , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
				$pointer_text .= '<p>' . esc_js( sprintf( __('This version %s improves class assignement in navigation menus when using menu insertion points.','xili-language') , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';

				$pointer_text .= '<p>' . esc_js( sprintf( __('Previous version before v. %s introduced new functions like is_xili_curlang() to test current language in theme.','xili-language') , XILILANGUAGE_VER) ). '</p>';
