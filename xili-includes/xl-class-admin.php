	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %s adds new way/option to use flag of media asset. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';

				$pointer_text .= '<p>' . esc_js( sprintf( __( 'Previous version of %s improves plugin textdomain and mo file switching (more rules). See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
...
<?php
	...
	function flag_options_theme_menu () {
		...
		$field_args = array(
			...
			'id'			=> 'flags_list',
			'name'			=> 'flags_list',
			'desc'			=> __("The list of images uploaded and assigned as flag in Media table.", "xili-language"),
			'std'			=> 'with-flag',
