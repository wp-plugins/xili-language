	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __('This version %s incorporate a new feature : Another menu w/o location can be inserted everywhere in nav menu (according current language of webpage).','xili-language') , XILILANGUAGE_VER) ). '</p>';
				
				$pointer_text .= '<p>' . esc_js( sprintf( __('Previous version of %s incorporate a new feature : List of a sub-selection of pages (according current language of webpage) can be inserted everywhere in nav menu. A powerful way to create very dynamic menus!','xili-language') , XILILANGUAGE_VER) ). '</p>';
	 ...
	 */
	 function add_help_text( $contextual_help, $screen_id, $screen ) { 
	 	...
	 	if ( $screen->id == 'nav-menus' ) { // 2.8.8
	      ...
	      '<ul>' .
		      '<li>' . __('After creating menu structures containing items linked to a language (but not assigned to a location), select a menu structure for each language in Menus selection box.','xili-language') . '</li>' .
