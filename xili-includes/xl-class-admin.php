	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %1$s now include improvements in image as flag management and detection. Admin side flags are now uploadable in Media Libray. This version %1$s is tested with 4.2-RC. See also %2$s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
