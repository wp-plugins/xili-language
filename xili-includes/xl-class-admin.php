...
class xili_language_admin extends xili_language {
	...
	function __construct( $xl_parent ){
		...
		add_action( 'admin_init', array(&$this,'set_author_rules_fields') );
	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %s incorporates authoring rules and settings in 5th tab. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
			$pointer_text .= '<p>' . esc_js( sprintf( __( '2nd tab of settings of this version %s is reorganized for front side adjustments.','xili-language' ) , XILILANGUAGE_VER) ). '</p>';
			$pointer_text .= '<p>' . esc_js( sprintf( __( 'Previous version before v. %s tests WP 3.9.','xili-language' ) , XILILANGUAGE_VER) ). '</p>';
...
// categories
	...
	function set_author_rules_fields () {
...
// $this->xili_settings['functions_enable']
	...
	function settings_authoring_settings_validate ( $input ){
		//error_log (serialize($this->xili_settings));
		...
		if ( count($thecustoms) > 0 ) {
			foreach ( array_keys ( $thecustoms ) as $cpt ) {
				$this->xili_settings['multilingual_custom_post'][$cpt]['multilingual'] = ( isset( $input['cpt_'.$cpt] ) ) ? $input['cpt_'.$cpt] : "" ; // because checkbox
			}
		} else {
		...
		switch ( $option_name ) {
			case $this->settings_authoring_settings:
				...
				if ( false !== strpos ( $id, 'cpt_') ) {
					...
					$options[$id] = $this->xili_settings['multilingual_custom_post'][$cpt]['multilingual'];
				}
				// Bookmarks
...
/******************************* LINKS ****************************/
	...
	function add_help_text( $contextual_help, $screen_id, $screen ) {
		...
		if ( in_array ( $screen->id , array ('settings_page_language_page', 'settings_page_language_front_set', 'settings_page_language_expert', 'settings_page_language_files', 'settings_page_author_rules', 'settings_page_language_support') ) ) {
			...
			'<li>' . sprintf ( __('If the option %s is checked, available translations from child are merged with those from parent. The parent translation (if also available in child) has priority in this case.','xili-language'), "<em>" . __( 'MO merging between parent and child','xili-language') . "</em>" ) ;
			;
