...
class xili_language_admin extends xili_language {
	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %s improves xml import and importations from GlotPress. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
			$pointer_text .= '<p>' . esc_js( sprintf( __( 'Previous version before v. %s incorporates authoring rules and settings in 5th tab. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
			$pointer_text .= '<p>' . esc_js( __( '2nd tab of settings was reorganized for front side adjustments.','xili-language' ) ). '</p>';
		...
		<div id="posttype-xlmenulist" class="posttypediv">
			<div id="tabs-panel-xlmenulist" class="tabs-panel tabs-panel-active tabs-panel-menu">
				<ul id ="xlmenulist-checklist" class="categorychecklist form-no-clear">
					...
					if ( $nav_menus_wo ) { // now saves slug
						echo '<li>';
