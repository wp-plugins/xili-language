<?php
/**
 * class xili_language_admin - 2.6.3 - 2.7.1
 *
 */
...
class xili_language_admin extends xili_language {
	...
	function on_box_frontend ( $data ) { 
		...
		<fieldset style="<?php echo $rightboxstyle ?>"><legend><?php _e('Select language of the home page', 'xili-language'); ?></legend>
		...
		<fieldset style="<?php echo $rightboxstyle ?>"><legend><?php _e("Theme's language when categories in 'all'", 'xili-language'); ?></legend>
		...
		?>
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php _e('Multilingual custom posts', 'xili-language'); ?></legend>
		...
		if ( current_theme_supports( 'widgets' ) ) {  // theme widget enable
			...
			?>	
			<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php _e('Bookmarks widget settings', 'xili-language'); ?></legend>
	...
	function on_sidebox_for_specials ( $data ) {
		if ($this->lang_perma ) { // to force permalinks flush ?> 
			...
			<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php _e('Permalinks rules', 'xili-language'); ?></legend>
		...
		<?php } ?>
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php _e('Translation domains settings', 'xili-language'); ?></legend><p>
		...
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php _e('Dashboard style: External xl-style.css', 'xili-language'); ?></legend>
		<?php
		...
		<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php _e('Locale (date) translation', 'xili-language'); ?></legend><p>
	 	...
	 	<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php _e('Redirect to created post', 'xili-language'); ?></legend><p>
		...
		if ( function_exists('is_child_theme') && is_child_theme() ) { // 1.8.1 and WP 3.0
	 	...
	 	<fieldset style="margin:2px 2px 5px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo __("Theme type and domain:",'xili-language'); ?></legend>
	 		...
	 		if ("" != $this->parent->thetextdomain) {
	 			...
	 			echo '<span style="color:red">'.$this->admin_messages['alert']['no_domain_defined'].'</span>';
	 			if (''!=$this->domaindetectmsg) { 
	 				echo '<br /><span style="color:red">'. $this->domaindetectmsg.' '.$this->admin_messages['alert']['default'].'</span>';
	 			}
	 		} ?><br />
	 		</p>
	 	</fieldset>
	 	...
	 	if ( $this->parent->ltd === false )	echo '<br /><span style="color:red">'.$this->admin_messages['alert']['no_load_function'].'</span>'; 
	 	...
	 	<fieldset style="margin:2px; padding:12px 6px; border:1px solid #ccc;"><legend><?php echo __("Theme's nav menu items settings",'xili-language'); ?></legend>
			...
			<br />	<br />
		 	...
		 	if($list_in_nav_enable =='enable') {
		 			echo '<br /><span style="color:red">'.$this->admin_messages['alert']['menu_auto_inserted'].'</span>'; }
	...
	function on_box_mail_content ( $data ) {
		...
		<?php } ?>
		<fieldset style="margin:2px; padding:12px 100px 12px 30px; border:1px solid #ccc;"><legend><?php _e('Mail to dev.xiligroup', 'xili-language'); ?></legend><p style="text-align:right;">
	...
	function on_sidebox_uninstall_content ( $data ) {
		extract( $data );
	?>
	<p style="color:red;"><?php echo $this->admin_messages['alert']['plugin_deinstalling']; ?></p>
	...
	function on_box_lang_form_content( $data ) { 
		...
		<h2 id="addlang" <?php if ($action=='delete') echo 'style="color:#FF1111;"'; ?>><?php echo $formtitle ;  ?></h2>
		<?php if ($action=='edit' || $action=='delete') :?>
		...
		<table class="editform" width="100%" cellspacing="2" cellpadding="5">
			...
			<?php if ( $action=='delete' ) :?>
			<tr>
				<th scope="row" valign="top" align="right"><label for="multilingual_links_erase"><span style="color:red" ><?php echo $this->admin_messages['alert']['erasing_language']; ?></span></label>&nbsp;:&nbsp;</th>
	 ...
	 *
	 */
	 function print_styles_options_language_page ( ) {
	...
	function available_mo_files( $path , $filename ) {
  		...
  		$alert = '<span style="color:red;">'.__('Uncommon filename','xili-language').'</span>' ;
