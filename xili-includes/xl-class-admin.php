	...
	function localize_admin_js( $case_news, $news_id ) {
 		...
 		switch ( $case_news ) {
 			...
 			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __('This version %s fixes issues and add more texts.','xili-language') , XILILANGUAGE_VER) ). '</p>';

				$pointer_text .= '<p>' . esc_js( sprintf( __('Previous version %s incorporate a new feature : Sub-selection of menus (according current language of webpage) can be inserted everywhere in nav menu (insertion point method). A powerful way to create very dynamic menus!','xili-language') , XILILANGUAGE_VER) ). '</p>';
			...
			case 'languages_settings':
				...
				$pointer_text .= '<p>' . esc_js( sprintf(__( 'Before to question dev.xiligroup support, do not forget to visit %s documentation', 'xili-language' ), '<a href="http://wiki.xiligroup.org" title="'.$about.'" >wiki</a>' ) ). '</p>';
 				...
 				//$pointer_Offset = '-110 -13';
 				//$pointer_my = 'top right';
 				//$pointer_at = 'right top';
				break;
