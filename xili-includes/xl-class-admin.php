	...
	function localize_admin_js( $case_news, $news_id ) {
		...
		switch ( $case_news ) {
			...
			case 'xl_new_version' :
				...
				$pointer_text .= '<p>' . esc_js( sprintf( __( 'This version %1$s now include improvements in image as flag management and detection. Admin side flags are now uploadable in Media Libray. This version %1$s is tested for 4.2-beta. See also %2$s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';

				$pointer_text .= '<p>' . esc_js( sprintf( __( 'Previous version %s is ready for 4.1 “Dinah” new install with twenty fifteen theme. See also %s.','xili-language' ) , XILILANGUAGE_VER, '<a href="http://wordpress.org/plugins/xili-language/changelog/" title="'.$changelog.'" >'.$changelog.'</a>') ). '</p>';
	...
	function author_rules() {
		...
		if ( current_theme_supports( 'widgets' ) ) {
			...
			foreach ( $this->xili_settings['specific_widget'] as $key => $value ) {
				...
				'desc'			=> sprintf( __('Checked, this widget of class %s is available.', 'xili-language'), $key ),
				'std'			=> 'enabled',
...
<?php
	...
	function flag_theme_options_help_page () {
		...
		$help .= '<p>'.__('Here, you define the use of frontend flags.', 'xili-language').'</p>';
		$help .= '<p>'. sprintf(__('More detailled infos in future version and here %s.', 'xili-language'), $this->wikilink ).'</p>';
